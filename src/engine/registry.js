...
// ---------------------------------------------------------------------------
// Compressible flow helpers (Fanno / isothermal)
// ---------------------------------------------------------------------------

/**
 * Fanno parameter Φ(Ma, γ).
 * Monotonically decreasing for Ma in (0,1), Φ(1) = 0.
 * Φ = (1−Ma²)/(γ·Ma²) + (γ+1)/(2γ)·ln[(γ+1)·Ma²/(2+(γ−1)·Ma²)]
 */
export function fannoParameter(Ma, gamma) {
  const Ma2 = Ma * Ma;
  return (1 - Ma2) / (gamma * Ma2) +
    (gamma + 1) / (2 * gamma) *
    Math.log((gamma + 1) * Ma2 / (2 + (gamma - 1) * Ma2));
}

/**
 * Invert Φ(Ma) by bisection to find Ma given a target Φ value.
 * Returns 1.0 if targetPhi ≤ 0 (choked).
 */
export function solveFannoMach(targetPhi, gamma) {
  if (targetPhi <= 0) return 1.0;
  let lo = 1e-8, hi = 1.0 - 1e-12;
  for (let i = 0; i < 50; i++) {
    const mid = (lo + hi) / 2;
    const phi = fannoParameter(mid, gamma);
    if (phi > targetPhi) {
      lo = mid;
    } else {
      hi = mid;
